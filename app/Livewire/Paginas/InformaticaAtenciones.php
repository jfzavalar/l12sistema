...
class InformaticaAtenciones extends Component
{
    ...
    public function render()
    {
        $lista_activos = $this->queryConFiltros()
            ...
            ->select('personales_atenciones.*')

            ->where('personales_atenciones.activo', 1)
            ->where('personales_atenciones.dni', $this->search)

            ->orderBy('personales_atenciones.id', 'desc')

            ->paginate(5,['*'],'atencionesPage'
            );
        ...
        return view('livewire.paginas.informatica-atenciones',
            ...
            ->when($this->search, function ($query) {
                ...
                $query->where(function ($q) use ($search) {
                    ...
                    if (preg_match('/^\d{8}$/', $search)) {
                        // Si es un DNI completo de 8 dígitos
                        $q->where('personales_atenciones.dni', $search);
                    } else {
                        // Si no es un DNI, buscar por nombres
                        $q->where('personales_atenciones.datos', 'like', '%' . $search . '%');
                    }
    ...
    public function guardar()
    {
        ...
        try {
            ...
            $this->reset(['servicio_id','servicio','detalle_servicio','detalle_problema']);
