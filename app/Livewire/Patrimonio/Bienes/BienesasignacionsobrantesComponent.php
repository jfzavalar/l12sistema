...
use App\Models\PatrimoniosBienesSobrantesAsignacionesDetalle;
...
class BienesasignacionsobrantesComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    {
        $lista_activos = PatrimoniosBienesSobrantesAsignacione::select(
                'id',
                'persona_id',
                'dni',
                'personal_id',
        ...
        $lista_bienes = PatrimoniosBienesSobrante::where('activo','1')
            ->where('codigo_patrimonial','like','%' . $this->searchbienes . '%')
            ->distinct()
            ->orderBy('descripcion')
            ->paginate(10,['*'],'bienesPage');

        return view('livewire.patrimonio.bienes.bienesasignacionsobrantes-component',
    ...
    public function guardar()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                // ✅ Crear cabecera
                $iasignacion = PatrimoniosBienesSobrantesAsignacione::create([
                ...
                $yaAsignados = PatrimoniosBienesSobrante::whereIn('id', $ids)
                ...
                // ✅ Insertar detalles
                PatrimoniosBienesSobrantesAsignacionesDetalle::insert($detalles);

                // ✅ Update sin validación incorrecta
                PatrimoniosBienesSobrante::whereIn('id', $ids)
    ...
    public function editar($id)
    {
        ...
        $asignacion = PatrimoniosBienesSobrantesAsignacione::findOrFail($id);
        ...
        // ===== DETALLES =====
        $this->bienes = PatrimoniosBienesSobrantesAsignacionesDetalle::where('asignacion_id', $id)
            ->join('patrimonios_bienes_sobrantes as pb', 'pb.id', '=', 'patrimonios_bienes_sobrantes_asignaciones_detalles.bien_id')
            ->get()
            ->map(function ($item) {
                return [
                    'id' => $item->bien_id,
    ...
    public function actualizar()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                $asignacion = PatrimoniosBienesSobrantesAsignacione::findOrFail($this->asignacion_id);

                // =========================
                // 🔥 OBTENER BIENES ANTERIORES
                // =========================
                $bienesAntiguos = PatrimoniosBienesSobrantesAsignacionesDetalle::where('asignacion_id', $asignacion->id)
                    ->pluck('bien_id')
                    ->toArray();
                ...
                if (!empty($bienesAntiguos)) {
                    PatrimoniosBienesSobrante::whereIn('id', $bienesAntiguos)
                ...
                // 🗑️ ELIMINAR DETALLES
                // =========================
                PatrimoniosBienesSobrantesAsignacionesDetalle::where('asignacion_id', $asignacion->id)->delete();
    ...
    public function agregar_bien(PatrimoniosBienesSobrante $ibien)
    {
        // Datos del bien
        $item = [
            'id' => $ibien->id,
